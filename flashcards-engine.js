/* Versa Concursos — agendamento local de flashcards v1.2.0. */
(() => {
  'use strict';
  ...
  function normalizeStore(raw) {
    ...
    return {
      ...
      sessions: Array.isArray(source.sessions) ? source.sessions.slice(-60) : []
    };
  }
  ...
  function normalizeCard(raw) {
    ...
    return {
      due: source.due || null,
      intervalDays: Number.isFinite(source.intervalDays) ? Math.max(0, source.intervalDays) : 0,
      ease: Number.isFinite(source.ease) ? Math.min(3.2, Math.max(1.3, source.ease)) : 2.5,
      repetitions: Number.isInteger(source.repetitions) ? Math.max(0, source.repetitions) : 0,
      lapses: Number.isInteger(source.lapses) ? Math.max(0, source.lapses) : 0,
      reviewCount: Number.isInteger(source.reviewCount) ? Math.max(0, source.reviewCount) : 0,
  ...
  function schedule(raw, rating, nowValue = Date.now()) {
    if (!RATINGS.has(rating)) throw new Error(`Classificação inválida: ${rating}`);
    ...
    if (rating === 'again') {
      ...
      next.intervalDays = previous.repetitions
        ? Math.max(2, Math.round(Math.max(previous.intervalDays, 1) * next.ease))
        : 3;
      next.repetitions += 1;
      wait = next.intervalDays * DAY;
    } else {
      ...
      next.intervalDays = previous.repetitions
        ? Math.max(4, Math.round(Math.max(previous.intervalDays, 1) * (next.ease + 0.5)))
        : 7;
      next.repetitions += 1;
      wait = next.intervalDays * DAY;
    }
